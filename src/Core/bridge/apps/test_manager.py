...
class ManagerActionsTest(unittest.TestCase):
    ...
    def test_steam_game_launcher_must_be_uninstalled_in_steam(self):
        ...
        self.assertEqual(info["label"], "Desinstalar pela Steam")
        self.assertIn("desinstalado na Steam", info["reason"])
    ...
    def test_steam_game_size_uses_manifest_size_on_disk(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            app = {
                ...
                "exec": "steam steam://rungameid/413150",
                "icon": "steam_icon_413150",
                "desktop_file": str(library / "Stardew Valley.desktop"),
                "source": "user",
            }
        ...
        self.assertEqual(size["label"], "676.1 MB")
        self.assertEqual(size["source"], "steam:413150")
    ...
    def test_uninstall_flatpak_runs_flatpak_uninstall(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run_mock.assert_called_once_with(
                ["flatpak", "uninstall", "--assumeyes", "--app", "com.example.App"],
                check=True,
                stdout=manager.subprocess.DEVNULL,
                stderr=manager.subprocess.PIPE,
                text=True,
            )
            self.assertEqual(result["target"], "com.example.App")

    def test_system_package_uninstall_is_available_when_owner_is_known(self):
        with mock.patch.object(manager, "package_owner", return_value="brave-bin"):
            info = manager.uninstall_info({"id": "brave-browser.desktop", "desktop_file": "/usr/share/applications/brave-browser.desktop", "source": "system"})

        self.assertTrue(info["can"])
        self.assertEqual(info["method"], "system-package")
        self.assertEqual(info["package"], "brave-bin")
    ...
    def test_uninstall_system_package_runs_pkexec_pacman(self):
        ...
        run_mock.assert_called_once_with(
            ["pkexec", "pacman", "-Rns", "--noconfirm", "brave-bin"],
            check=True,
            stdout=manager.subprocess.DEVNULL,
            stderr=manager.subprocess.PIPE,
            text=True,
