...
Rectangle {
    ...
    property int horizontalPadding: text === "" ? 0 : Components.Theme.spacingLarge
    ...
    readonly property color hoverBg: isLight ? Qt.rgba(0, 0, 0, 0.055) : Qt.rgba(1, 1, 1, 0.10)
    readonly property color pressedBg: isLight ? Qt.rgba(0, 0, 0, 0.08) : Qt.rgba(1, 1, 1, 0.13)

    signal clicked()

    implicitWidth: controlWidth > 0 ? controlWidth : (text === "" ? implicitHeight : Math.max(minWidth, labelRow.implicitWidth + horizontalPadding * 2))
    ...
    Row {
        ...
        Text {
            ...
            text: root.iconText
            color: root.enabled
                ? (root.primary ? root.foregroundColor : (root.text === "" ? root.mutedForegroundColor : root.foregroundColor))
