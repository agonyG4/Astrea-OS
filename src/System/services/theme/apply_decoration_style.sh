...
conf_lua="${HOME}/.config/hypr/ui/decoration.lua"
rules_lua="${HOME}/.config/hypr/system/rules/windowrules.lua"
lock_dir="${HOME}/.cache/Astrea"
tmp_files=()

mkdir -p "${lock_dir}"
exec 9>"${lock_dir}/theme-decoration.lock"
flock 9
...
if [[ -f "${conf_lua}" ]]; then
    ...
    awk \
        ...
        -v blur_vibrancy="${blur_vibrancy}" \
        -v blur_vibrancy_darkness="${blur_vibrancy_darkness}" \
        -v blur_xray="${blur_xray}" \
        -v blur_popups_ignorealpha="${blur_popups_ignorealpha}" '
function set_lua_value(line, key, value) {
    if (line ~ "^[[:space:]]*" key "[[:space:]]*=") {
        match(line, /^[[:space:]]*/)
        return substr(line, RSTART, RLENGTH) key " = " value ","
    }
    return line
}
...
{
    line = $0
    line = set_lua_value(line, "active_opacity", active_opacity)
    line = set_lua_value(line, "inactive_opacity", inactive_opacity)
    line = set_lua_value(line, "dim_strength", dim_strength)

    if (line ~ "^[[:space:]]*blur[[:space:]]*=[[:space:]]*[{]") {
        in_blur = 1
    } else if (in_blur && line ~ "^[[:space:]]*[}],[[:space:]]*$") {
        in_blur = 0
    } else if (in_blur) {
        line = set_lua_value(line, "enabled", blur_enabled)
        line = set_lua_value(line, "size", blur_size)
        line = set_lua_value(line, "passes", blur_passes)
        line = set_lua_value(line, "noise", blur_noise)
        line = set_lua_value(line, "contrast", blur_contrast)
        line = set_lua_value(line, "brightness", blur_brightness)
        line = set_lua_value(line, "vibrancy", blur_vibrancy)
        line = set_lua_value(line, "vibrancy_darkness", blur_vibrancy_darkness)
        line = set_lua_value(line, "xray", blur_xray)
        line = set_lua_value(line, "popups_ignorealpha", blur_popups_ignorealpha)
    }
...
' "${conf_lua}" > "${tmp}"
    ...
    commit_tmp "${tmp}" "${conf_lua}"
fi

if [[ -f "${rules_conf}" ]]; then
    tmp="$(make_tmp_for "${rules_conf}")"
    awk -v layer_xray="${quickshell_layer_xray}" '
    function shellbar_blur_rule(namespace, spacing) {
        return "layerrule = blur on," spacing "match:namespace " namespace
    }
    function layer_blur_rule(namespace, spacing) {
        return "layerrule = blur on," spacing "match:namespace " namespace
    }
    ...
    $0 ~ /^[[:space:]]*layerrule[[:space:]]*=[[:space:]]*blur[[:space:]]+(on|off),.*match:namespace[[:space:]]+astrea-bar[[:space:]]*$/ {
        print shellbar_blur_rule("astrea-bar", "  ")
        if (layer_xray == "on")
            print topbar_xray_rule("astrea-bar", "  ")
        next
    }

    $0 ~ /^[[:space:]]*layerrule[[:space:]]*=[[:space:]]*blur[[:space:]]+(on|off),.*match:namespace[[:space:]]+bar[[:space:]]*$/ {
        print shellbar_blur_rule("bar", "  ")
        if (layer_xray == "on")
            print topbar_xray_rule("bar", "  ")
        next
    }
    $0 ~ /^[[:space:]]*layerrule[[:space:]]*=[[:space:]]*blur[[:space:]]+(on|off),.*match:namespace[[:space:]]+topbar-popup[[:space:]]*$/ {
        print layer_blur_rule("topbar-popup", " ")
        next
    }
    $0 ~ /^[[:space:]]*layerrule[[:space:]]*=[[:space:]]*blur[[:space:]]+(on|off),.*match:namespace[[:space:]]+astrea-notifications[[:space:]]*$/ {
        print layer_blur_rule("astrea-notifications", " ")
        next
    }
    $0 ~ /^[[:space:]]*layerrule[[:space:]]*=[[:space:]]*blur[[:space:]]+(on|off),.*match:namespace[[:space:]]+volume-osd[[:space:]]*$/ {
        print layer_blur_rule("volume-osd", " ")
        next
    }
    $0 ~ /^[[:space:]]*layerrule[[:space:]]*=[[:space:]]*ignore_alpha[[:space:]]+[^,]+,.*match:namespace[[:space:]]+volume-osd[[:space:]]*$/ {
        print "layerrule = ignore_alpha 0.05, match:namespace volume-osd"
        next
    }
    { print }
    ' "${rules_conf}" > "${tmp}"
    commit_tmp "${tmp}" "${rules_conf}"
fi

if [[ -f "${rules_lua}" ]]; then
    tmp="$(make_tmp_for "${rules_lua}")"
    awk -v layer_xray="${quickshell_layer_xray}" '
    function topbar_xray_rule(namespace) {
        if (layer_xray == "on") {
            return "layer(\"" namespace "\", { xray = true })"
        }
        return ""
    }

    $0 ~ /^[[:space:]]*layer\("(astrea-bar|bar|topbar-popup|astrea-notifications|volume-osd)",[[:space:]]*[{][[:space:]]*xray[[:space:]]*=[[:space:]]*true[[:space:]]*[}]\)[[:space:]]*$/ {
        next
    }

    $0 ~ /^[[:space:]]*layer\("astrea-bar",[[:space:]]*[{][[:space:]]*blur[[:space:]]*=[[:space:]]*(true|false)[[:space:]]*[}]\)[[:space:]]*$/ {
        print "layer(\"astrea-bar\", { blur = true })"
        if (layer_xray == "on")
            print topbar_xray_rule("astrea-bar")
        next
    }

    $0 ~ /^[[:space:]]*layer\("bar",[[:space:]]*[{][[:space:]]*blur[[:space:]]*=[[:space:]]*(true|false)[[:space:]]*[}]\)[[:space:]]*$/ {
        print "layer(\"bar\", { blur = true })"
        if (layer_xray == "on")
            print topbar_xray_rule("bar")
        next
    }
