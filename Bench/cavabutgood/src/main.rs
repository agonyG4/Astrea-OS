...
fn normalize_bands(raw: &[f32], adaptive_peak: &mut f32) -> Vec<f32> {
    ...
    // The original visualizer normalized each frame against its own peak.
...
fn find_music_monitor() -> String {
    ...
    if let Ok(out) = output {
        ...
        if let Some((score, sink_id)) = best {
            if let Some(monitor) = sink_monitor_name(&sink_id) {
                eprintln!(
                    "[visualizer] Stream de música detectado no sink {} (score {}): {}",
    ...
    eprintln!("[visualizer] Stream de música não detectado, usando monitor padrão");
    eprintln!("[visualizer] Dica: toque algo no player antes de iniciar o visualizer");
...
fn main() {
    ...
    let output = {
        ...
        eprintln!("[visualizer] Modo socket + stdout");
    ...
    let pa = Simple::new(
        ...
        Some(&buffer_attr),              // baixa latência para o visualizer
    ).unwrap_or_else(|e| {
