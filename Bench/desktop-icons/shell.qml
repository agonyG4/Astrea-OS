...
ShellRoot {
    ...
    readonly property string scriptPath: modulePath + "/generate_apps.py"
    ...
    Variants {
        ...
        delegate: PanelWindow {
            ...
            WlrLayershell.namespace: "bench-desktop-icons"
            ...
            function openContextAt(x, y, app, background) {
                ...
                contextMenu.menuX = Math.min(Math.max(8, x), Math.max(8, contextHost.width - contextMenu.menuWidth - 8))
                contextMenu.menuY = Math.min(Math.max(8, y), Math.max(8, contextHost.height - contextMenu.height - 8))
            ...
            function slotIndexFor(x, y) {
                ...
                var rows = Math.max(1, Math.ceil(usableHeight / root.cellHeight))
                var rowStep = rows > 1 ? (usableHeight - root.cellHeight) / (rows - 1) : root.cellHeight
                var col = Math.max(0, Math.round((x - left - root.cellWidth / 2) / root.cellWidth))
            ...
            Rectangle {
                            ...
                            MouseArea {
                                ...
                                onReleased: function(mouse) {
                                    if (mouse.button === Qt.LeftButton) {
                                        ...
                                        if (moved) {
                                            var center = tile.mapToItem(contextHost, tile.width / 2, tile.height / 2)
                                            if (tile.appData)
                                                root.setDesktopSlot(tile.appData.desktop, desktopWindow.slotIndexFor(center.x, center.y))
                                        }
                                        tile.dragging = false
                                        tile.movedDuringDrag = false
                                    }
                ...
                Common.ContextMenuPopup {
                    ...
                    Common.ContextMenuAction {
                        ...
                        visible: root.contextIsBackground || root.hasGridPositions()
                        actionEnabled: true
                        onTriggered: {
