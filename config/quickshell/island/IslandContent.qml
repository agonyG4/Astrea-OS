...
Rectangle {
    ...
    width: {
        ...
        if (island.idleHidden) return island.isExpanded ? 360 : 120
        return island.isExpanded ? 360 : 180
    }
    height: {
        if (isGamemodeNotify) return 100
        if (!island.hasMusic) return island.isExpanded ? 38 : 34
        if (island.idleHidden) return island.isExpanded ? 160 : 34
    ...
    Item {
        anchors.fill: parent
        visible: opacity > 0
        opacity: (!islandContent.isGamemodeNotify && island.hasMusic && !island.idleHidden && !islandContent.isMouseOver) ? 1 : 0
            ...
            Item {
                ...
                Image {
                    ...
                    layer.effect: OpacityMask {
                        maskSource: Rectangle { width: 20; height: 20; radius: 10; antialiasing: true }
                    }
                }
            }
        }
