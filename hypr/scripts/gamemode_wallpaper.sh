...
case "$1" in
    start)
        ...
        # Caso o systemctl não funcione, mata direto pra garantir
        pkill -x linux-wallpaperengine >> "$LOG_FILE" 2>&1
        ;;
    end)
        # 1. Reinicia o Wallpaper Engine
        echo "Iniciando linux-wallpaperengine.service" >> "$LOG_FILE"
        systemctl --user start linux-wallpaperengine.service >> "$LOG_FILE" 2>&1
        
        # 2. Espera um pouco para o engine carregar antes de matar o swww
        (sleep 3 && pkill -x "swww-daemon") &
        ;;
esac
