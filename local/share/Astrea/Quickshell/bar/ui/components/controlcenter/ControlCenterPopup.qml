...
SystemComponents.TopbarPopup {
    ...
    function toggleBluetooth() {
        btPowerProc.turnOn = !control.btOn
    ...
    Process {
        id: btPowerProc
        property bool turnOn: true
        command: ["python3", Quickshell.env("HOME") + "/.local/share/Astrea/System/scripts/bluetooth_manager.py", "power", turnOn ? "on" : "off"]
        running: false
        onExited: {
            if (!control.btProcess) return
            control.btProcess.refresh()
            if (turnOn && control.shown)
                control.btProcess.requestScan("control-center")
        }
    }
