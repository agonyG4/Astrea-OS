...
Item {
    ...
    readonly property string _sharedBase: Quickshell.env("HOME") + "/.local/share/Astrea/System/Paper"
    readonly property string _base:       _sharedBase + "/LockscreenWallpaper"
    readonly property string wpFull:      _base + "/wallpaper.jpg"
    readonly property string wpName_f:    _base + "/wallpaper_name.txt"
    readonly property string userDir:     _sharedBase + "/UserWallpapers"
    readonly property string imgCache:    _sharedBase + "/scripts/img_cache.py"
    readonly property string _thumbPath:  _base + "/wallpaper_thumb.jpg"

    property string wpThumb: "file://" + _thumbPath + "?t=" + Date.now()
    property string wpName:   "Lockscreen Wallpaper"
    ...
    Process {
        id: ensureDirProc
        running: false
        command: ["sh", "-c", "mkdir -p " + JSON.stringify(root._base) +
                  " && mkdir -p " + JSON.stringify(root.userDir)]
    }

    Process {
        id: initProc
        running: false
        command: ["sh","-c", "cat " + JSON.stringify(root.wpName_f) + " 2>/dev/null"]
        stdout: SplitParser {
            onRead: (l) => { if (l.trim()) root.wpName = l.trim() }
        }
    }

    Process {
        id: copyWallpaperProc; running: false
        function run(src) {
    ...
    Process {
        id: blurProc; running: false
        command: ["python3", root._sharedBase + "/scripts/blur_lockscreen.py"]
    }
    ...
    Process {
        ...
        command: ["python3", root.imgCache, "--only-lockscreen"]
        running: false
    ...
    Process {
        ...
        command: ["python3", root.imgCache, "--lockscreen-slug", slug]
        onExited: () => {
            root._picked = ""
            root.scanUsers()
        }
