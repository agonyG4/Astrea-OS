...
Item {
    ...
    readonly property string decorationApplyPath: (Quickshell.env("HOME") || "") + "/.local/share/Astrea/System/services/apply_theme_decoration.sh"
    ...
    Process {
        ...
        function save() {
            ...
            command = ["bash", "-c",
                ...
                "if [ -x \"$3\" ]; then \"$3\" \"$2\"; fi\n",
                "--", theme.configPath, String(theme.shellStyle), theme.decorationApplyPath]
            running = false
            running = true
        }
    }
