...
## Top Level
- `Apps`: About, Explorer, Settings, Weather.
...
- `System`: auth helper, service scripts, portal backend, config, launch backend, metadata.
...
- [[Astrea - Explorer App]]: `Apps/Explorer/Main.qml`; tabs, list/icon views, file actions, portal dialog, Quick Look handoff.
...
## Core And Features
- `Core/components`: shared QML controls, forms, menus, navigation, typography, theme, feedback, and progress surfaces.
- `Core/bridge`: forwarding entrypoints plus app, audio, display, network, storage, system, and wallpaper domain backends.
